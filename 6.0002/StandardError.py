...
def getDiffs(population, sampleSizes):
    ...
    return pylab.array(diffsFracs)

def getSDestimate(population, sampleSizes):
    popStd = numpy.std(population)
    ...
    for sampleSize in sampleSizes:
        ...
        for t in range(100):
            sample = random.sample(population, sampleSize)
            SDhat.append( sampleSize/(sampleSize-1)*numpy.std(sample) )
            
        SD = sum(SDhat)/len(SDhat)
        SDestimates.append(SD)
        
    return pylab.array(SDestimates)
...
sampleSizes = range(20, 600, 10)
diffs = getSDestimate(getHighs(), sampleSizes)
