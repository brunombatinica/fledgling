...
def main():
    ...
    #load in hyperparemeters
    hp = Hyperparameters()
    
    # Load data
    #df = feather.read_dataframe(hp.data_dir + 'Py_VARIANZ_2012_v3-1.feather')
    df = pd.read_feather(hp.data_dir + 'raw_data/VARIANZ_2014.feather')
    
    # Exclude na's
    df.dropna(subset=['end_fu_time'], inplace=True)
    ...
    df['ph_antiplat_anticoag_prior_6mths'] = df['ph_antithrombotic_prior_6mths'].astype(bool)
    
    # Combine ethnicities back together
    df['en_prtsd_eth'].replace({4:9, 42:9, 5:9}, inplace=True)
    
    # Create antiplatelet/anticoagulant column #ALREADY DONE
    # df['ph_antiplat_anticoag_prior_6mths'] = df['ph_antiplatelets_prior_6mths'] | df['ph_anticoagulants_prior_6mths']
    
    # df['EVENT'] = df["event"]
    # Time to event and binary event column #ALREADY DONE 
    df["TIME"] = df["end_fu_time"].astype(int)
  
    #df['end_fu_date'] = pd.to_datetime(df['end_fu_date'], format='%Y-%m-%d', errors='coerce')
    # df['EVENT_DATE'] = df[['out_broad_cvd_adm_date', 'end_fu_date']].min(axis=1) #Both are the same
    ...
    num_censored_5y_males = (1-df.loc[df['gender_code'] & (df['TIME'] == 1825), 'EVENT']).sum() #TIME = 1826 but EVENT = 0 CHECK THAT THIS WORKS WITH MY DATA
    num_censored_5y_females = (1-df.loc[~df['gender_code'] & (df['TIME'] == 1825), 'EVENT']).sum()
