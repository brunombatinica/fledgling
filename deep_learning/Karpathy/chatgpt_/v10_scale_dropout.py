...
# hyperparameters
batch_size = 64 #number of independent sequences
block_size = 256 #maximum context length of our prediciton \
# (in reality only the last element is used for the bigram model)
n_layer = 6
dropout = 0.2
head_size = 16
n_head = 6
n_embd = 384 # number of embedding demensions
max_iters = 5000
eval_interval = 300
learing_rate = 3e-4
...
# LAYER NORM IS ALREADY IMPLEMENTED IN PYTORCH

# class LayerNorm(nn.Module): #
#     #Layer norm - normalizes our features -  
#     # mean and variance are taken over hte embed
#     # makes features unit gaussian at initialization 
#     # but them learns how to scale and shift them

#     def __init__(self, n_embd, eps = 1e-5):
#         super().__init__()
#         self.gamma = nn.Parameter(torch.ones(n_embd)) #gain - ?each feature or each example in hte
#         self.beta = nn.Parameter(torch.zeros(n_embd)) #bias - ?each feature or each example in hte
...
class Head(nn.Module):
    ...
    def forward(self,x):
        ...
        wei = self.dropout(wei) #dropout the attention scores "affinities"
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,x):
        ...
        out = self.dropout(self.proj(out)) #lienar transofrmation of hte output of the MHA
...
class Block(nn.Module):
    ...
    def __init__(self, n_embd, n_head):
        ...
        self.ln1 = nn.LayerNorm(n_embd) #need 2 different ones as gamma and beta are learned
        self.ln2 = nn.LayerNorm(n_embd) # SEE IMPLEMENTATION IN SELF ATTENION
        # batch and time act as batch dimensions
        # per token transformation
        # Normalizes the features of each token
        # so each token has a unit norm distribution at initialization

    def forward(self, x):
        x = x + self.sa( self.ln1(x)) #This is the residual connection
...
class AttentionLanguageModel(nn.Module):
    def __init__(self):
        ...
        self.blocks = nn.Sequential(*[Block(n_embd, n_head=n_head) for _ in range(n_layer)]) # trained n_layer transformer blocks [B,T,C] - each block is a transformer block
        self.ln_f = nn.LayerNorm(n_embd), #normalizes the output of the transformer block
    ...
    def forward(self, idx, targets=None):  
        ...
        x = self.ln_f(x) #(B,T,C) #normalizes the output of the transformer block 
