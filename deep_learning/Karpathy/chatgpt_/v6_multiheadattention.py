...
class AttentionLanguageModel(nn.Module):
    def __init__(self):
        ...
        self.mha_head = MultiHeadAttention(n_head, head_size)
    ...
    def forward(self, idx, targets=None):  
       ...
       # bp()
        ...
        x = self.mha_head(x) #(B,head_size*num_heads,C)
        x = self.ffwd(x) #(B,T,C) gives each token "time" to "think" on the attention result
